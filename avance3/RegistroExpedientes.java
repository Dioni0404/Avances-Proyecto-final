...
public class RegistroExpedientes {
    ...
    public void agregar(Empleado empleado, Expediente expediente) {
        ...
        historial.agregar(expediente.getCodDOC(),expediente.getAsuntoDOC(),expediente.getEstadoDOC(),expediente.getCodUSER(), expediente.getFecha(),
                expediente.getDestinoEMP(), expediente.getDestinoArea());
        ...
        while (aux != null) {
            if(aux.getCodEMP() == empleado.getCodEMP()){
                while(!aux.getCola().esVacia()){
                    ...
                    contador++;
                    if(contador == 8){
                        if(destinoEMP==" " || destinoEMP == aux.getCodEMP()|| destinoEMP==""){ // OJO solo vacio para indicar que no se ha enviado
                            colaaux.encolar(aux.getCola().desencolar()); // Encolamos DestinoArea
                            historial(colaaux);
                            while(!colaaux.esVacia()){
                                colanoderivada.encolar(colaaux.desencolar());
                            }
                        }else{
                            colaaux.encolar(aux.getCola().desencolar()); // Encolamos DestinoArea
                            historial(colaaux); // Guardamos el historial de envio
                            NodoSobrecargado aux1 = cabeza;
                            while(aux1!=null){
                                if(aux1.getCodEMP() == destinoEMP){
                                    while(!colaaux.esVacia()){
                                        aux1.getCola().encolar(colaaux.desencolar());
                                    }
                                }
...
//            String CodDOC, String AsuntoDOC, String EstadoDOC, String CodUSER, String Fecha, String DestinoEMP, String DestinoArea
            historial.agregar(codDoc,AsuntoDoc,EstadoDOC,codUSER,Fecha,DestinoEMP,DestinoArea);   
        }
    }
